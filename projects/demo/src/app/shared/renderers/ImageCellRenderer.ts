...
@Component({
  ...
  template: `
    @if (value()) {
      <img class="table-image" [src]="value()!" [alt]="alt()" loading="lazy" (error)="onImageError($event)" />
    } @else {
      <span class="image-placeholder">بدون تصویر</span>
    }
  `,
  styles: `
    .table-image { display: block; width: 56px; height: 40px; border-radius: 6px; object-fit: cover; }
    .image-placeholder { font-size: 12px; opacity: 0.6; }
  `,
})
export class ImageCellRenderer<T extends object> implements CellRendererComponent<string | null, T> {
