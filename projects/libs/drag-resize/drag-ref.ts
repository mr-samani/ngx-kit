...
export class DragRef<T = unknown> {
  ...
  nudge(dx: number, dy: number): void {
    ...
    if (!wasDragging) this.startDrag(this.lastPointer);
    this.dragMove({ x: this.startPointer.x + this.moveDx + dx, y: this.startPointer.y + this.moveDy + dy });
  }
  ...
  /** Aborts the drag and snaps the element back to its origin without emitting a drop. */
  cancelDrag(): void {
    if (!this.isDragging()) return;
    this.moveDx = 0;
    this.moveDy = 0;
  ...
  private applyTransform(): void {
    const base = this.previousTransform && this.previousTransform !== 'none' ? this.previousTransform + ' ' : '';
