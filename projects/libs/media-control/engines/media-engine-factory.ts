 ...
 * decision is not made here; `NgxMediaFacade.resolveEngine()` tries native
 * first and falls back to the decoder registry on a genuine (format-level)
 * failure. This class stays responsible only for constructing engines.
 *
 * Synchronous on purpose: constructing a `NativeMediaEngine` never awaits
 * anything, so wrapping it in a `Promise` bought nothing but an extra
 * microtask on every load. Callers that still `await` this are unaffected.
 */
...
export class NgxMediaEngineFactory {
  ...
  create(
    ...
    return new NativeMediaEngine(kind, runOutsideAngular, undefined, preload);
  }
